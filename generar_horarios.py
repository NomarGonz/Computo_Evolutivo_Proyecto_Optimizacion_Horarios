    ...
    for dia, horarios in horarios_semanales.items():
        if horarios:
            ...
            for inicio, fin, clave, seccion in horarios:
                nombre_materia = df_filtrado[df_filtrado['Clave'] == clave]['Materia'].iloc[0]
                print(f"  {clave} - {nombre_materia} - Sección: {seccion}, de {inicio.strftime('%I:%M %p')} a {fin.strftime('%I:%M %p')}")
...
for generacion in range(num_generaciones):
    ...
    mejor = min(poblacion, key=lambda cromo: funcion_fitness(cromo, df_filtrado, claves_semestre))


#Imprimir el horario del mejor cromosoma
...
print("Total de horas muertas por semana")
print(funcion_fitness(mejor_cromosoma,df_filtrado, claves_semestre))
